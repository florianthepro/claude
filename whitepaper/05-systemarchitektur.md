...
Die Architektur besteht aus sechs Schichten. Innerhalb des Stapels S6 - S4 - S3 - S2 - S1 ruft jede Schicht nur die unmittelbar darunterliegende auf; das Überspringen einer Stapelschicht ist kein Optimierungsspielraum, sondern ein Fehler, weil es die Rechte- und Fehlerabgrenzung aufhebt, aus der sich die Invarianten ergeben. S2b (fremdgepflegte Datenebene) und S5 (Konnektoren) sind Seitenflächen und nicht Teil dieser Reihenfolge; sie werden über je einen benannten Kanal angesprochen, S2b aus S4 über 8404/tcp xDS+SDS, S5 aus S3 über den lokalen Unix-Socket des Konnektors. Der Eintritt von S6 läuft über den Eingang, der als Teil von S2b den Verkehr terminiert und an S4 weiterreicht; auch das ist kein Sprung im Stapel, sondern der eine benannte Weg über eine Seitenfläche.
...
| **Eingang (Envoy)** | L7-Terminierung, Routen, SNI-Listener je Mandant ab M1 | C++ (fremdgepflegt) | Benutzer `atrium-eingang`, leere Capability-Menge; die Bindung an 80/tcp, 443/tcp und 443/udp erfolgt über Socketaktivierung durch systemd | 80, 443/tcp, 443/udp eingehend; 8404 ausgehend zum Kern | Nur der zuletzt empfangene, versionierte Konfigurationsschnappschuss im Speicher; keine Datei | Behält beim Verlust der xDS-Verbindung den letzten Stand bei (fail-static); ein Fehler beim Anwenden führt zum Verwerfen des neuen Schnappschusses, nicht zum Leerzustand | Neustart bricht bestehende Verbindungen; deshalb nur bei Abbildwechsel mit Verbindungsentleerung, nie zur Konfigurationsänderung |
...
| `atrium-eingang.service` | `atrium-eingang` | keine | Die Bindung an 80/tcp, 443/tcp und 443/udp geschieht nicht im Prozess: systemd öffnet die Sockets und übergibt sie dem Prozess bereits gebunden (Socketaktivierung), sodass keine Capability nötig ist | Nichts; insbesondere keine eigene Portbindung, keine weitere Socketöffnung unterhalb 1024 |
